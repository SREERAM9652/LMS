...
const Announcements = () => {
  ...
  const createAnnouncement = async () => {
    ...
    try {
      const response = await axios.post(`${API_BASE}/api/announcements/create`, payload, {
    ...
    <>
      <style>
        {`
          ...
          .container { max-width: 1200px; margin: auto; padding: 20px; margin-left:250px; }
          .announcements-title { font-size: 2.5rem; font-weight: bold; color: #007bff; text-align: left; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1.5px; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.1); margin-right:300px; }
          .announcement-card { width: 100%; max-width: 500px; border-radius: 12px; transition: transform 0.3s ease-in-out, box-shadow 0.3s ease-in-out; background-color: #ffffff; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.15); margin: auto; text-align: center; overflow: hidden; padding: 20px; margin-bottom: 20px; margin-top:20px; margin-right:400px; }
          .announcement-card:hover { transform: scale(1.05); box-shadow: 0 6px 12px rgba(0, 0, 0, 0.2); }
          .announcement-input { width: 80%; padding: 10px; border-radius: 6px; border: 1px solid #ccc; margin-right: 10px; }
          .btn-primary { background-color: #007bff; border: none; border-radius: 6px; padding: 10px 15px; font-size: 1rem; transition: background-color 0.3s; }
          .btn-primary:hover { background-color: #0056b3; }
          .btn-danger { background-color: #dc3545; border: none; border-radius: 6px; padding: 10px 15px; font-size: 1rem; }
          .btn-danger:hover { background-color: #c82333; }
          .announcement-actions { display: flex; justify-content: center; gap: 10px; margin-top: 10px; }
          @media screen and (max-width: 768px) { .main-content { margin-left: 0; padding: 15px; } .announcement-card { max-width: 100%; } .announcement-input { width: 100%; } }
        `}
      </style>
